...
// Modified by:
// Created:     02/04/05
// RCS-ID:      $Id: mediactrl_gst.cpp,v 1.1 2006-02-15 20:19:10 ryannpcs Exp $
...
//#define wxGSTREAMER_VISUALIZATIONS
#ifdef wxGSTREAMER_VISUALIZATIONS
#   define wxVisualizationSize wxSize(320,240)
#else
#   define wxVisualizationSize wxSize(0,0)
#endif
...
bool wxGStreamerMediaBackend::QueryVideoSizeFromElement(GstElement* element)
{
    ...
    if(list == NULL)
    {
        m_videoSize = wxVisualizationSize; 
        return false;
    }
...
//-----------------------------------------------------------------------------
// wxGStreamerMediaBackend::SetupVisualization
//
// Sets up the GStreamer visualization
//
//-----------------------------------------------------------------------------
#if GST_VERSION_MAJOR == 0 && GST_VERSION_MINOR <= 8
void wxGStreamerMediaBackend::SetupVisualization()
{
    GList* features;
    GList* visuals = NULL;
    ...
    regsmaster = gst_registry_pool_list();
    regs = regsmaster;
    while(regs)
    {
        plugins = GST_REGISTRY(regs->data)->plugins;
        while(plugins)
        {
            features = gst_plugin_get_feature_list(GST_PLUGIN(plugins->data));
            while(features)
            {
                ...
                if(GST_IS_ELEMENT_FACTORY(feature))
                {
                    ...
                    if(wxString(factory->details.klass, wxConvUTF8).Find(
                            wxT("Visualization")) != (int)wxString::npos)
                    {
                        visuals = g_list_append(visuals, 
    ...
    g_list_free(regsmaster);

    wxString sMsg = wxT("VISUALS:\n");
        
    GList* vistemp = visuals;
    while(vistemp)
    {
        sMsg = wxString((char*)vistemp->data, wxConvUTF8) + wxT("\n");
        vistemp = vistemp->next;
    }
    wxLogDebug(sMsg);
    GstElement* element = 
        gst_element_factory_make((gchar*)visuals->data, 
                                 "vis_plugin_element");
    g_object_set(m_playbin, "vis-plugin", element, NULL);
    g_list_free(visuals);
}
#else
...
{
    //clear previous
    g_object_set(m_playbin, "vis-plugin", NULL, NULL);

    GList* features;
    GList* featuresmaster;
    GstElement* element = NULL;

    featuresmaster = features = gst_registry_get_feature_list(
            ...
            while(features)
            {
               ...
               gst_plugin_feature_list_free(featuresmaster);
...
#if GST_VERSION_MAJOR > 0 || GST_VERSION_MINOR >= 10
    GError* error;
    if(!gst_init_check(&wxTheApp->argc, &wxTheApp->argv, &error))
    {
...
#else
    if(!gst_init_check(&wxTheApp->argc, &wxTheApp->argv))
    {
        wxLogSysError(wxT("Could not initialize GStreamer"));
        return false;
    }
...
#if GST_VERSION_MAJOR > 0 || GST_VERSION_MINOR >= 10
#include "wx/dynlib.h"
typedef int (*LPgnome_vfs_tell)(void* handle, gint64* pSize);

wxLongLong wxGStreamerMediaBackend::GetDownloadProgress()
{
    wxLongLong llSize = 0;
    GstElement* element;
        //Search for GnomeVFS source (may be using filesrc in which case
        //this is sort of inappropriate...)

    if(g_object_class_find_property(
            G_OBJECT_GET_CLASS(G_OBJECT(m_playbin)),
            "source") != NULL)
    {
        g_object_get(m_playbin, "source", &element, NULL);
    }
    else    
        element = gst_bin_get_by_name(GST_BIN(m_playbin), "source");
    
    if(element)
    {
        wxASSERT(GST_IS_ELEMENT(element));
        GstElementFactory* factory = gst_element_get_factory(element);
        if(wxString(gst_element_factory_get_longname(factory),
                    wxConvUTF8).Find(wxT("GnomeVFS")) != (int)wxString::npos)
        {
            wxDynamicLibrary gnomevfs;
            if(gnomevfs.Load(
            ...
            {
                LPgnome_vfs_tell pfnGnome_vfs_tell = 
                    (LPgnome_vfs_tell) gnomevfs.GetSymbol(
                        wxT("gnome_vfs_tell")
                                                          );
                if(pfnGnome_vfs_tell)
                {
                    void* handle;
                    g_object_get(element, "handle", &handle, NULL);
                    gint64 fs;
                    if((*pfnGnome_vfs_tell)(handle, &fs) == 0)
                    {
                        llSize = fs;
                    }//end gnomevfs tell
                }
                gnomevfs.Unload();
            }//end if gnomevfs dll loaded
        }//end if source is a GnomeVFS source
        g_object_unref(element);
    }//end if the source element exists  
...
wxLongLong wxGStreamerMediaBackend::GetDownloadProgress()
{
    ...
    if (!wxGst_element_query_position(m_playbin, &fmtBytes, &length) ||
