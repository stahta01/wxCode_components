...
: wxWindow(parent, id, pos, size, style|wxSIMPLE_BORDER)
{
  ...
  mMargin = 3;
  mGridSize = 10;
  mTextCtrl = new wxTextCtrl(this, id, value, wxDefaultPosition, wxDefaultSize,
...
void mmTextStretch::OnMouse(wxMouseEvent& event)
{
  ...
  if (event.Entering() && !mCtrlIsDown)
  {
    if ((MouseIsOnTopLeft() || MouseIsOnBottomRight()) && !mLeftIsDown)
    {
      if ((mStyle & mmTS_SIZE_TOP    && mStyle & mmTS_SIZE_LEFT) ||
	  (mStyle & mmTS_SIZE_BOTTOM && mStyle & mmTS_SIZE_RIGHT))
        SetCursor(*mSizeNWSECursor);
    }
    else
    if ((MouseIsOnTop() || MouseIsOnBottom())&& !mLeftIsDown)
    {
      if (mStyle & mmTS_SIZE_TOP || mStyle & mmTS_SIZE_BOTTOM)
        SetCursor(*mSizeNSCursor);
    }
    else
    if ((MouseIsOnLeft() || MouseIsOnRight())&& !mLeftIsDown)
    {
  ...
  if (event.LeftDown())
  {
    ...
    if (MouseIsOnBottomRight() && (mStyle & mmTS_SIZE_BOTTOM && mStyle & mmTS_SIZE_RIGHT))
      mBottomRightWasPressed = TRUE;
    if (MouseIsOnLeft() && (mStyle & mmTS_SIZE_LEFT))
      mLeftWasPressed = TRUE;
    else
    if (MouseIsOnRight() && (mStyle & mmTS_SIZE_RIGHT))
      mRightWasPressed = TRUE;
    else
    if (MouseIsOnTop() && (mStyle & mmTS_SIZE_TOP))
      mTopWasPressed = TRUE;
    else
    if (MouseIsOnBottom() && (mStyle & mmTS_SIZE_BOTTOM))
  ...
  if (event.Dragging() /*&& mLeftIsDown*/)
  {
    CaptureMouse();
    int mpx,mpy;
    ::wxGetMousePosition(&mpx,&mpy);
    GetParent()->ScreenToClient(&mpx,&mpy);
    if (mTopLeftWasPressed || mBottomRightWasPressed ||
	mTopWasPressed     || mBottomWasPressed ||
	mLeftWasPressed    || mRightWasPressed)
    {
      int x,y,w,h;
      wxSize  ts = GetSize();
      wxPoint tp = GetPosition();
	...
	else
	{
      ...
      if (mBottomRightWasPressed)
      {
	if (!(mStyle & mmTS_SNAP_TO_GRID) ||
            (abs(mpy-tp.y-ts.GetHeight())>mGridSize || abs(mpx-tp.x-ts.GetWidth())>mGridSize))
	{
	  x = tp.x;
	  y = tp.y;
          w = mpx-x;
	...
	if (!(mStyle & mmTS_SNAP_TO_GRID) || abs(mpy-tp.y)>mGridSize)
      ...
      if (mBottomWasPressed)
      {
	x = tp.x;
	if (mCtrlIsDown &&
	    (mStyle & mmTS_SNAP_TO_GRID) && abs(mpy-tp.y-ts.GetHeight())>mGridSize)
	  y = mpy-ts.GetHeight();
	else
	  y = tp.y;
        w = ts.GetWidth();
	...
	y = tp.y;
      ...
      {
	if (mCtrlIsDown &&
	    (mStyle & mmTS_SNAP_TO_GRID) && abs(mpx-tp.x-ts.GetWidth())>mGridSize)
	  x = mpx-ts.GetWidth();
	else
	  x = tp.x;
	y = tp.y;
