...
// Constructor
{
  ...
  if (prevDbl)
  {
    mPrevDblButton = new mmMultiButton(this, mmID_NAV_PREV2, "",
	                               *gPrevDblBitmap,
  ...
  if (prev)
  {
    mPrevButton = new mmMultiButton(this, mmID_NAV_PREV, "",
	                            *gPrevBitmap,
			            wxDefaultPosition, wxDefaultSize,
				    mmMB_AUTODRAW);
  }
  ...
  if (next)
  {
    mNextButton = new mmMultiButton(this, mmID_NAV_NEXT, "",
	                            *gNextBitmap,
              		            wxDefaultPosition, wxDefaultSize,
              			    mmMB_AUTODRAW);
  }
  ...
  if (nextDbl)
  {
    mNextDblButton = new mmMultiButton(this, mmID_NAV_NEXT2, "",
	                               *gNextDblBitmap,
              		               wxDefaultPosition, wxDefaultSize,
              			       mmMB_AUTODRAW);
  }
...
// Destructor
{
  if (mPrevDblButton) { mPrevDblButton->Destroy(); mPrevDblButton=NULL; }
  if (mPrevButton   ) { mPrevButton->Destroy();    mPrevButton   =NULL; }
  if (mNextButton   ) { mNextButton->Destroy();    mNextButton   =NULL; }
  if (mNextDblButton) { mNextDblButton->Destroy(); mNextDblButton=NULL; }
} // Destructor
...
void mmNavigator::OnButton(wxCommandEvent& event)
{
  ...
  switch (event.GetId())
  {
    case mmID_NAV_PREV2: GenerateEvent(mmEVT_NAVIGATOR_PREV2);break;
    case mmID_NAV_PREV:  GenerateEvent(mmEVT_NAVIGATOR_PREV); break;
    case mmID_NAV_NEXT:  GenerateEvent(mmEVT_NAVIGATOR_NEXT); break;
    case mmID_NAV_NEXT2: GenerateEvent(mmEVT_NAVIGATOR_NEXT2);break;
  }
} // OnButton
