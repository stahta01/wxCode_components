...
// Author:      original code by: Alex Andruschak
...
wxBorderItemHandle::wxBorderItemHandle( unsigned long ln )
{
    location_id = ln;
    
    if (ln == wxNORTH) location_string = wxT("North");
	else if (ln == wxSOUTH) location_string = wxT("South");
	else if (ln == wxWEST) location_string = wxT("West");
	else if (ln == wxEAST) location_string = wxT("East");
	else if (ln == wxCENTER) location_string = wxT("Center");
	else
	{
...
void wxBorderSizer::RecalcSizes()
{
    ...
    while (current != NULL)
	{
        ...
        if (item != NULL && (location = (wxBorderItemHandle *)item->GetUserData()) != NULL)
		{
            ...
            // North
            if (location->GetLocationCode() == wxNORTH)
			{
				// minimal x and y
                c_point.x = pos.x;
                c_point.y = pos.y;
            ...
            // South
            if (location->GetLocationCode() == wxSOUTH)
			{
				// minimal x, maximal y
                c_point.x = pos.x;
            ...
            // West
            if (location->GetLocationCode() == wxWEST)
			{
				// minimal x and y
                c_point.x = pos.x;
            ...
            // East
            if (location->GetLocationCode() == wxEAST)
			{
				// maximal x, minimal y
            ...
            // Center
            if (location->GetLocationCode() == wxCENTER)
			{
				// minimal x and y
