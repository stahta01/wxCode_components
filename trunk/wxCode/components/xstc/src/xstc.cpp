...
{//Sets up default behavior, links events and calls default coloring
	...
	linemgn = 0; //Mapped to a margin
	symmgn = 1;
	fldmgn = 2;
	linew = 30; //Width for the respective margin
	symw = 15;
	fldw = 15;
	SetMargins(3,2); //right and left padding margins
	...
	c_ext_array[0] = wxT("c"); //Fast lookup of C/C++ extentions
	c_ext_array[1] = wxT("h");
	...
	cppcase = false; //Case sensetive lexers for clw/cpp
	clwcase = false;
#ifndef XSTC_NO_CONFIG
	useext  = false; //Use extention to lexer config
	usecolor  = false; //Use config colors
	useprops = false; //Use config properties
...
#ifndef XSTC_NO_KEYS
	usekeys = false; //Use automatic keyword loading
	InitKeys();
#endif
	colorstyle = wxT("null"); //Current loaded theme
	XFilename = wxT("Untitled"); //Internal filename
	braceline = true; //Indent guide linkes matched braces

#ifndef XSTC_NO_CONFIG
	XSTCcolorDbase = NULL; //Color database, if color names are used in config this is necessary.
#endif //XSTC_NO_CONFIG

	for(int x=0; x<10; x++)
	{
		BmarkHandles[x] = -1; //Numbered bookmark handles
	}
	CStyles = wxT(""); //Names of all config based themes, provided for convienience.

	colorconf = NULL; //Config database for colors and extentions to lexer mapping

	ResetStyle(); //Initializes the editor to default colors
	SetProperty(wxT("fold"), wxT("1")); //Activate folding by default
...
void XSTC::MarginClicked(XSTC_EVENT& event)
{
	...
	//Check if it was bookmark or fold margin
	if (event.GetMargin() == symmgn)
	{
		ToggleBookMark(line);
	}
	else if (event.GetMargin() == fldmgn)
	{
...
{
	//Marker 0 is the main click to toggle bookmark.
...
void XSTC::MarkerToggle(int marker, bool move, int newline)
{//Numbered bookmarks
	if(marker >= 0 && marker <= 9)
	{
		if(move)
		{
			...
			if(newline >= 0)
			{
				//We want to move the marker not delete it.
				if(BmarkHandles[marker] != -1)
				{
					//We have a valid marker
					MarkerDeleteHandle(BmarkHandles[marker]);
					if(LineFromPosition(GetCurrentPos()) == newline)
					{
						//We are on the same line so delete the marker
						BmarkHandles[marker] = -1;
					}
					else
					{
						//Move the marker
						BmarkHandles[marker] = MarkerAdd(newline, marker + 1); //since the numbered bookmarks start on marker 1 we need to adjust
					}
				}
				else
				{
			...
			{
				//We are told to delete marker if it exsists
				if(BmarkHandles[marker] != -1)
				{
					//We have a valid marker
					MarkerDeleteHandle(BmarkHandles[marker]);
		...
		else
		{
			//Goto ALT+markernum
			if(BmarkHandles[marker] != -1)
				if(MarkerLineFromHandle(BmarkHandles[marker]) >= 0)
					GotoLine(MarkerLineFromHandle(BmarkHandles[marker]));
		}
	}
	//Else do nothing because we don't have a valid marker number.
}
...
void XSTC::KeyParse(wxKeyEvent& event)
{
	...
	if(!event.ShiftDown() && (mod == wxMOD_ALT || mod == wxMOD_CONTROL) && (x >= 48 && x <= 57))
	{//Check for valid marker toggle keys
		switch(x)
		{
			case 48:
			{
		...
		if(move)
		{
			MarkerToggle(x, move, LineFromPosition(GetCurrentPos()));
		}
		else
			MarkerToggle(x, move);
	}
...
{//Sets lexer and any default properties, or looks them up from config.

	//Make the fold margin visible so that we can turn it off if a lexer does not support it.
	...
	switch(lexer)
	{
		...
		case XSTC_DEF(LEX_POWERPRO):
		{
			...
			if(usekeys)
			{
				...
				this->SetKeyWords(1,KeyCheck(Keys_Set.powerpro1));
...
long XSTC::LoadFileX(wxString filename, bool setlexer)
{//Loads a file and optionally sets the lexer for its file extention.
	bool result = false;
	result = LoadFile(filename);
	XFilename = filename;
	if(setlexer && result)
	{
		SetLexerX(AutoEXT(filename));
		return FILE_OK;
	}
	else if(setlexer && !result)
	{
		CloseFile(); //Since we were going to replace the current file anyway.
...
long XSTC::SaveFileX(const wxString filename, long saveas)
{//Extends Scintilla's save function with our auto-run functionality and optionally a save as dialog.
	long status = FILE_NOT_FOUND; //Results we return
	...
	if(filename == wxT(""))
		usexfilename = true;
		/*When an empty filename is passed the internal filename is used.
		 *Internally "XFilename" is used as the argument automatically,
		 *the variable is reused since we only need to know the filename initially.
		 */
	...
	else if(saveas == SAVE_AS_ON_FAIL)
	{
		if(usexfilename)
		{
	...
	else if(saveas == SAVE_AS_NEVER)
	{
		if(usexfilename)
		{
			if(XFilename != wxT("Untitled"))
			{
				usexfilename = this->SaveFile(XFilename);
				if(usexfilename)
				{
...
{//Check file extension and return the applicable lexer
	filename.MakeLower();
	filename.Trim(true);
	filename.Trim(false);
	wxString filext = wxT("");
...
#endif //XSTC_NO_ADA
#ifndef XSTC_NO_APDL
	else if(filext == wxT("mac"))
	{
		return XSTC_DEF(LEX_APDL);
	}
#endif //XSTC_NO_APDL
#ifndef XSTC_NO_ASM
	else if(filext == wxT("asm"))
	{
		return XSTC_DEF(LEX_ASM);
	}
#endif //XSTC_NO_ASM
#ifndef XSTC_NO_ASN1
	else if(filext == wxT("mib"))
	{
		return XSTC_DEF(LEX_ASN1);
	}
#endif //XSTC_NO_ASN1
#ifndef XSTC_NO_ASYMPTOTE
	else if(filext == wxT("asy"))
	{
		return XSTC_DEF(LEX_ASYMPTOTE);
	}
#endif //XSTC_NO_ASYMPTOTE
#ifndef XSTC_NO_AU3
	else if(filext == wxT("au3"))
	{
		return XSTC_DEF(LEX_AU3);
	}
#endif //XSTC_NO_AU3
#ifndef XSTC_NO_AVE
	else if(filext == wxT("ave"))
	{
		return XSTC_DEF(LEX_AVE);
	}
#endif //XSTC_NO_AVE
#ifndef XSTC_NO_BAAN
	else if(filext == wxT("bc") || filext == wxT("cln"))
	{
		return XSTC_DEF(LEX_BAAN);
	}
#endif //XSTC_NO_BAAN
#ifndef XSTC_NO_BLITZBASIC
	else if(filext == wxT("bb"))
	{
		return XSTC_DEF(LEX_BLITZBASIC);
	}
#endif //XSTC_NO_BLITZBASIC
#ifndef XSTC_NO_FREEBASIC
	else if(filext == wxT("bas") || filext == wxT("bi"))
	{
		return XSTC_DEF(LEX_FREEBASIC);
	}
#endif //XSTC_NO_FREEBASIC
#ifndef XSTC_NO_POWERBASIC
	else if(filext == wxT("powerbasic"))
	{
		return XSTC_DEF(LEX_POWERBASIC);
	}
#endif //XSTC_NO_POWERBASIC
#ifndef XSTC_NO_PUREBASIC
	else if(filext == wxT("pb"))
	{
		return XSTC_DEF(LEX_PUREBASIC);
	}
#endif //XSTC_NO_PUREBASIC
#ifndef XSTC_NO_BASH
	else if(filext == wxT("sh") || filext == wxT("bsh") || filext == wxT("configure"))
	{
		return XSTC_DEF(LEX_BASH);
	}
#endif //XSTC_NO_BASH
#ifndef XSTC_NO_BATCH
	else if(filext == wxT("bat") || filext == wxT("cmd") || filext == wxT("nt"))
	{
		return XSTC_DEF(LEX_BATCH);
	}
#endif //XSTC_NO_BATCH
#ifndef XSTC_NO_BULLANT
	else if(filext == wxT("ant"))
	{
		return XSTC_DEF(LEX_BULLANT);
	}
#endif //XSTC_NO_BULLANT
#ifndef XSTC_NO_CAML
	else if(filext == wxT("ml") || filext == wxT("mli"))
	{
		return XSTC_DEF(LEX_CAML);
	}
#endif //XSTC_NO_CAML
#ifndef XSTC_NO_CMAKE
	else if(filext == wxT("cmake"))
	{
		return XSTC_DEF(LEX_CMAKE);
	}
#endif //XSTC_NO_CMAKE
#ifndef XSTC_NO_COBOL
	else if(filext == wxT("cob"))
	{
		return XSTC_DEF(LEX_COBOL);
	}
#endif //XSTC_NO_COBOL
#ifndef XSTC_NO_CONF
	else if(filext == wxT("conf"))
	{
		return XSTC_DEF(LEX_CONF);
	}
#endif //XSTC_NO_CONF
#ifndef XSTC_NO_CSOUND
	else if(filext == wxT("orc") || filext == wxT("sco") || filext == wxT("csd"))
	{
		return XSTC_DEF(LEX_CSOUND);
	}
#endif //XSTC_NO_CSOUND
#ifndef XSTC_NO_CSS
	else if(filext == wxT("css"))
	{
		return XSTC_DEF(LEX_CSS);
	}
#endif //XSTC_NO_CSS
#ifndef XSTC_NO_D
	else if(filext == wxT("d"))
	{
		return XSTC_DEF(LEX_D);
	}
#endif //XSTC_NO_D
#ifndef XSTC_NO_DIFF
	else if(filext == wxT("diff") || filext == wxT("patch"))
	{
		return XSTC_DEF(LEX_DIFF);
	}
#endif //XSTC_NO_DIFF
#ifndef XSTC_NO_EIFFEL
	else if(filext == wxT("e"))
	{
		return XSTC_DEF(LEX_EIFFEL);
	}
#endif //XSTC_NO_EIFFEL
#ifndef XSTC_NO_EIFFELKW
	else if(filext == wxT("ek"))
	{
		return XSTC_DEF(LEX_EIFFELKW);
	}
#endif //XSTC_NO_EIFFELKW
#ifndef XSTC_NO_ERLANG
	else if(filext == wxT("erl"))
	{
		return XSTC_DEF(LEX_ERLANG);
	}
#endif //XSTC_NO_ERLANG
#ifndef XSTC_NO_ERRORLIST
	else if(filext == wxT("err"))
	{
		return XSTC_DEF(LEX_ERRORLIST);
	}
#endif //XSTC_NO_ERRORLIST
#ifndef XSTC_NO_ESCRIPT
	else if(filext == wxT("src") || filext == wxT("em"))
	{
		return XSTC_DEF(LEX_ESCRIPT);
	}
#endif //XSTC_NO_ESCRIPT
#ifndef XSTC_NO_F77
	else if(filext == wxT("f77"))
	{
		return XSTC_DEF(LEX_F77);
	}
#endif //XSTC_NO_F77
#ifndef XSTC_NO_FLAGSHIP
	else if(filext == wxT("prg"))
	{
		return XSTC_DEF(LEX_FLAGSHIP);
	}
#endif //XSTC_NO_FLAGSHIP
#ifndef XSTC_NO_FORTH
	else if(filext == wxT("forth"))
	{
		return XSTC_DEF(LEX_FORTH);
	}
#endif //XSTC_NO_FORTH
#ifndef XSTC_NO_FORTRAN
	else if(filext == wxT("f") || filext == wxT("for") || filext == wxT("f90") || filext == wxT("f95") || filext == wxT("f2k"))
	{
		return XSTC_DEF(LEX_FORTRAN);
	}
#endif //XSTC_NO_FORTRAN
#ifndef XSTC_NO_GAP
	else if(filext == wxT("g"))
	{
		return XSTC_DEF(LEX_GAP);
	}
#endif //XSTC_NO_GAP
#ifndef XSTC_NO_GUI4CLI
	else if(filext == wxT("cli"))
	{
		return XSTC_DEF(LEX_GUI4CLI);
	}
#endif //XSTC_NO_GUI4CLI
#ifndef XSTC_NO_HASKELL
	else if(filext == wxT("haskell"))
	{
...
#endif //XSTC_NO_HTM
#ifndef XSTC_NO_INNOSETUP
	else if(filext == wxT("iss"))
	{
		return XSTC_DEF(LEX_INNOSETUP);
	}
#endif //XSTC_NO_INNOSETUP
#ifndef XSTC_NO_CPP
	else if(filext == wxT("js"))
	{
		return XSTC_DEF(LEX_CPP);
	}
	else if(filext == wxT("java"))
	{
		return XSTC_DEF(LEX_CPP);
	}
#endif //XSTC_NO_CPP
#ifndef XSTC_NO_KIX
	else if(filext == wxT("kix"))
	{
		return XSTC_DEF(LEX_KIX);
	}
#endif //XSTC_NO_KIX
#ifndef XSTC_NO_LATEX
	else if(filext == wxT("latex"))
	{
		return XSTC_DEF(LEX_LATEX);
	}
#endif //XSTC_NO_LATEX
#ifndef XSTC_NO_LISP
	else if(filext == wxT("lsp") || filext == wxT("lisp"))
	{
		return XSTC_DEF(LEX_LISP);
	}
#endif //XSTC_NO_LISP
#ifndef XSTC_NO_LOT
	else if(filext == wxT("lot"))
	{
		return XSTC_DEF(LEX_LOT);
	}
#endif //XSTC_NO_LOT
#ifndef XSTC_NO_LOUT
	else if(filext == wxT("lt"))
	{
		return XSTC_DEF(LEX_LOUT);
	}
#endif //XSTC_NO_LOUT
#ifndef XSTC_NO_LUA
	else if(filext == wxT("lua"))
	{
		return XSTC_DEF(LEX_LUA);
	}
#endif //XSTC_NO_LUA
#ifndef XSTC_NO_MAKEFILE
	else if(filext == wxT("makefile") || filext == wxT("mak"))
	{
		return XSTC_DEF(LEX_MAKEFILE);
	}
#endif //XSTC_NO_MAKEFILE
#ifndef XSTC_NO_MATLAB
	else if(filext == wxT("m"))
	{
		return XSTC_DEF(LEX_MATLAB);
	}
#endif //XSTC_NO_MATLAB
#ifndef XSTC_NO_METAPOST
	else if(filext == wxT("mp") || filext == wxT("mpx") || filext == wxT("mpy"))
	{
		return XSTC_DEF(LEX_METAPOST);
	}
#endif //XSTC_NO_METAPOST
#ifndef XSTC_NO_MMIXAL
	else if(filext == wxT("mms"))
	{
		return XSTC_DEF(LEX_MMIXAL);
	}
#endif //XSTC_NO_MMIXAL
#ifndef XSTC_NO_NIMROD
	else if(filext == wxT("nim"))
	{
		return XSTC_DEF(LEX_NIMROD);
	}
#endif //XSTC_NO_NIMROD
#ifndef XSTC_NO_NNCRONTAB
	else if(filext == wxT("tab") || filext == wxT("spf"))
	{
		return XSTC_DEF(LEX_NNCRONTAB);
	}
#endif //XSTC_NO_NNCRONTAB
#ifndef XSTC_NO_NSIS
	else if(filext == wxT("nsi") || filext == wxT("nsh"))
	{
		return XSTC_DEF(LEX_NSIS);
	}
#endif //XSTC_NO_NSIS
#ifndef XSTC_NO_OCTAVE
	else if(filext == wxT("octave"))
	{
		return XSTC_DEF(LEX_OCTAVE);
	}
#endif //XSTC_NO_OCTAVE
#ifndef XSTC_NO_OPAL
	else if(filext == wxT("impl") || filext == wxT("sign"))
	{
		return XSTC_DEF(LEX_OPAL);
	}
#endif //XSTC_NO_OPAL
#ifndef XSTC_NO_PASCAL
	else if(filext == wxT("pas") || filext == wxT("pp") || filext == wxT("inc") || filext == wxT("dpk") || filext == wxT("dfm") || filext == wxT("dpr"))
	{
		return XSTC_DEF(LEX_PASCAL);
	}
#endif //XSTC_NO_PASCAL
#ifndef XSTC_NO_PERL
	else if(filext == wxT("pl") || filext == wxT("pm") || filext == wxT("cgi") || filext == wxT("pod"))
	{
		return XSTC_DEF(LEX_PERL);
	}
#endif //XSTC_NO_PERL
#ifndef XSTC_NO_PO
	else if(filext == wxT("po"))
	{
		return XSTC_DEF(LEX_PO);
	}
#endif //XSTC_NO_PO
#ifndef XSTC_NO_POV
	else if(filext == wxT("pov") || filext == wxT("inc"))
	{
		return XSTC_DEF(LEX_POV);
	}
#endif //XSTC_NO_POV
#ifndef XSTC_NO_POWERPRO
	else if(filext == wxT("powerpro"))
	{
		return XSTC_DEF(LEX_POWERPRO);
	}
#endif //XSTC_NO_POWERPRO
#ifndef XSTC_NO_POWERSHELL
	else if(filext == wxT("ps1"))
	{
		return XSTC_DEF(LEX_POWERSHELL);
	}
#endif //XSTC_NO_POWERSHELL
#ifndef XSTC_NO_PROPERTIES
	else if(filext == wxT("inf") || filext == wxT("aut") || filext == wxT("cnf") || filext == wxT("cfg") || filext == wxT("url") || filext == wxT("reg") || filext == wxT("ini") || filext == wxT("properties"))
	{
		return XSTC_DEF(LEX_PROPERTIES);
	}
#endif //XSTC_NO_PROPERTIES
#ifndef XSTC_NO_PS
	else if(filext == wxT("ps"))
	{
		return XSTC_DEF(LEX_PS);
	}
#endif //XSTC_NO_PS
#ifndef XSTC_NO_PYTHON
	else if(filext == wxT("py") || filext == wxT("pyw"))
	{
		return XSTC_DEF(LEX_PYTHON);
	}
#endif //XSTC_NO_PYTHON
#ifndef XSTC_NO_R
	else if(filext == wxT("r"))
	{
		return XSTC_DEF(LEX_R);
	}
#endif //XSTC_NO_R
#ifndef XSTC_NO_REBOL
	else if(filext == wxT("reb"))
	{
		return XSTC_DEF(LEX_REBOL);
	}
#endif //XSTC_NO_REBOL
#ifndef XSTC_NO_RUBY
	else if(filext == wxT("rb") || filext == wxT("rbw"))
	{
		return XSTC_DEF(LEX_RUBY);
	}
#endif //XSTC_NO_RUBY
#ifndef XSTC_NO_SCRIPTOL
	else if(filext == wxT("sol"))
	{
		return XSTC_DEF(LEX_SCRIPTOL);
	}
#endif //XSTC_NO_SCRIPTOL
#ifndef XSTC_NO_SMALLTALK
	else if(filext == wxT("st"))
	{
		return XSTC_DEF(LEX_SMALLTALK);
	}
#endif //XSTC_NO_SMALLTALK
#ifndef XSTC_NO_SML
	else if(filext == wxT("sml"))
	{
		return XSTC_DEF(LEX_SML);
	}
#endif //XSTC_NO_SML
#ifndef XSTC_NO_SORCUS
	else if(filext == wxT("set"))
	{
		return XSTC_DEF(LEX_SORCUS);
	}
#endif //XSTC_NO_SORCUS
#ifndef XSTC_NO_SPECMAN
	else if(filext == wxT("e"))
...
#endif //XSTC_NO_SPECMAN
#ifndef XSTC_NO_SPICE
	else if(filext == wxT("scp") || filext == wxT("out") || filext == wxT("spice"))
	{
		return XSTC_DEF(LEX_SPICE);
	}
#endif //XSTC_NO_SPICE
#ifndef XSTC_NO_SQL
	else if(filext == wxT("sql") || filext == wxT("sp") || filext == wxT("sf") || filext == wxT("spb") || filext == wxT("sps") || filext == wxT("body") || filext == wxT("spec"))
	{
		return XSTC_DEF(LEX_SQL);
	}
#endif //XSTC_NO_SQL
#ifndef XSTC_NO_TACL
	else if(filext == wxT("tacl"))
	{
		return XSTC_DEF(LEX_TACL);
	}
#endif //XSTC_NO_TACL
#ifndef XSTC_NO_TADS3
	else if(filext == wxT("tads3"))
	{
		return XSTC_DEF(LEX_TADS3);
	}
#endif //XSTC_NO_TADS3
#ifndef XSTC_NO_TAL
	else if(filext == wxT("tal"))
	{
		return XSTC_DEF(LEX_TAL);
	}
#endif //XSTC_NO_TAL
#ifndef XSTC_NO_TCL
	else if(filext == wxT("tcl") || filext == wxT("itcl"))
	{
		return XSTC_DEF(LEX_TCL);
	}
#endif //XSTC_NO_TCL
#ifndef XSTC_NO_TEX
	else if(filext == wxT("tex") || filext == wxT("tuo") || filext == wxT("tui") || filext == wxT("idx") || filext == wxT("toc") || filext == wxT("aux") || filext == wxT("sty"))
	{
		return XSTC_DEF(LEX_TEX);
	}
#endif //XSTC_NO_TEX
#ifndef XSTC_NO_VB
	else if(filext == wxT("vb") || filext == wxT("vbp") || filext == wxT("frm") || filext == wxT("cls") || filext == wxT("clt") || filext == wxT("pag") || filext == wxT("dsr") || filext == wxT("dsm") || filext == wxT("dob"))
	{
		return XSTC_DEF(LEX_VB);
	}
#endif //XSTC_NO_VB
#ifndef XSTC_NO_VBSCRIPT
	else if(filext == wxT("vbs"))
	{
		return XSTC_DEF(LEX_VBSCRIPT);
	}
#endif //XSTC_NO_VBSCRIPT
#ifndef XSTC_NO_VERILOG
	else if(filext == wxT("v") || filext == wxT("vh"))
	{
		return XSTC_DEF(LEX_VERILOG);
	}
#endif //XSTC_NO_VERILOG
#ifndef XSTC_NO_VHDL
	else if(filext == wxT("vhd") || filext == wxT("vhdl"))
	{
		return XSTC_DEF(LEX_VHDL);
	}
#endif //XSTC_NO_VHDL
#ifndef XSTC_NO_XCODE
	else if(filext == wxT("xcode"))
	{
		return XSTC_DEF(LEX_XCODE);
	}
#endif //XSTC_NO_XCODE
#ifndef XSTC_NO_XML
	else if(filext == wxT("xml") || filext == wxT("xsl") || filext == wxT("xslt") || filext == wxT("svg") || filext == wxT("xul") || filext == wxT("xsd") || filext == wxT("dtd") || filext == wxT("xrc") || filext == wxT("axl"))
	{
		return XSTC_DEF(LEX_XML);
	}
#endif //XSTC_NO_XML
#ifndef XSTC_NO_YAML
	else if(filext == wxT("yml") || filext == wxT("yaml"))
	{
		return XSTC_DEF(LEX_YAML);
	}
#endif //XSTC_NO_YAML
//This is for plain text
	else if(filext == wxT("txt") || filext == wxT("nfo") || filext == wxT("diz") || filext == wxT("doc") || filext == wxT("rtf") || filext == wxT("lst") || filext == wxT("log") || filext == wxT("text"))
	{
		return thelexer;
	}
...
#ifndef XSTC_NO_CONFIG
void XSTC::FoldConf()
{//Check the config database for fold margin settings.
	wxString colorval, colorstr;
...
void XSTC::FoldBox()
{//Set up the fold margin with square type fold heads
...
{//Set up the fold margin with circle type fold heads
...
void XSTC::FoldArrow(bool lines)
{//Set up the fold margin with triangle type fold heads
	...
	{
		this->MarkerDefine(XSTC_DEF(MARKNUM_FOLDERMIDTAIL),XSTC_DEF(MARK_EMPTY), wxNullColour, wxNullColour);
		this->MarkerDefine(XSTC_DEF(MARKNUM_FOLDERSUB),    XSTC_DEF(MARK_EMPTY), wxNullColour, wxNullColour);
		this->MarkerDefine(XSTC_DEF(MARKNUM_FOLDERTAIL),   XSTC_DEF(MARK_EMPTY), wxNullColour, wxNullColour);
	}
...
void XSTC::FoldSimple(bool lines)
{//Set up the fold margin with +/- type fold heads
...
{//Set the fold margin style's colors
...
#ifndef XSTC_NO_CONFIG
void XSTC::StyleConf()
{//Configuration for non-language related styles
...
#ifndef XSTC_NO_MARK_LEFTRECT
	this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT),   wxT("fore:#FFFFFF,back:#000000"));
#endif //XSTC_NO_MARK_LEFTRECT
#ifndef XSTC_NO_MARK_AVAILABLE
	this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE),   wxT("fore:#FFFFFF,back:#000000"));
#endif //XSTC_NO_MARK_AVAILABLE
...
{//Emulates the VS theme in Dev-C++
...
{//Emulates the Classic theme in Dev-C++
...
{//Emulates the Borland turbo ide colors
	...
	this->StyleSetSpec(XSTC_DEF(MARK_ROUNDRECT),  wxT("fore:#FFFF00,back:#000080"));
	this->StyleSetSpec(XSTC_DEF(MARK_CHARACTER), wxT("fore:#FFFF00,back:#000080"));
#ifndef XSTC_NO_MARK_FULLRECT
	this->StyleSetSpec(XSTC_DEF(MARK_FULLRECT),   wxT("fore:#FFFF00,back:#000080"));
#endif //XSTC_NO_MARK_FULLRECT
#ifndef XSTC_NO_MARK_LEFTRECT
	this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT),   wxT("fore:#FFFF00,back:#000080"));
#endif //XSTC_NO_MARK_LEFTRECT
#ifndef XSTC_NO_MARK_AVAILABLE
	this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE),   wxT("fore:#FFFF00,back:#000080"));
#endif //XSTC_NO_MARK_AVAILABLE
...
void XSTC::JeffStyle()
{//A copy of a theme I found on the internet.
...
#endif //XSTC_NO_MARK_FULLRECT
#ifndef XSTC_NO_MARK_LEFTRECT
	this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT),   wxT("fore:#DFDFBF,back:#3F3F3F"));
#endif //XSTC_NO_MARK_LEFTRECT
#ifndef XSTC_NO_MARK_AVAILABLE
	this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE),   wxT("fore:#DFDFBF,back:#3F3F3F"));
#endif //XSTC_NO_MARK_AVAILABLE
...
{//A The Matrix like theme, but inspired partly by ZMatrix.
...
#endif //XSTC_NO_MARK_FULLRECT
#ifndef XSTC_NO_MARK_LEFTRECT
	this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT),   wxT("fore:#808080,back:#000000"));
#endif //XSTC_NO_MARK_LEFTRECT
#ifndef XSTC_NO_MARK_AVAILABLE
	this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE),   wxT("fore:#808080,back:#000000"));
#endif //XSTC_NO_MARK_AVAILABLE
...
//If style is one of the built in names call that function instead.
	if(style == wxT("null"))
	{
		ResetStyle();
		return;
	...
	if(colorconf)
	{
		if(colorconf->Read(path + wxT("CLBG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetCaretBk(color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CLBG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetCaretBk(color);
				else
		...
		if(colorconf->Read(path + wxT("FMC"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetFoldMarginColour(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/FMC"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetFoldMarginColour(true, color);
				else
		...
		if(colorconf->Read(path + wxT("FMHI"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetFoldMarginHiColour(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/FMHI"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetFoldMarginHiColour(true, color);
				else
		...
		if(colorconf->Read(path + wxT("WSFG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetWhitespaceForeground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WSFG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetWhitespaceForeground(true, color);
				else
					this->SetWhitespaceForeground(true, black);




		if(colorconf->Read(path + wxT("WSBG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetWhitespaceBackground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WSBG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetWhitespaceBackground(true, color);
				else
					this->SetWhitespaceBackground(true, white);




		if(colorconf->Read(path + wxT("CFG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetCaretForeground(color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CFG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetCaretForeground(color);
				else
					this->SetCaretForeground(black);




		if(colorconf->Read(path + wxT("SELFG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetSelForeground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/SELFG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetSelForeground(true, color);
				else
					this->SetSelForeground(true, white);




		if(colorconf->Read(path + wxT("SELBG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetSelBackground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/SELBG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetSelBackground(true, color);
				else
					this->SetSelBackground(true, black);




		if(colorconf->Read(path + wxT("BRKPTCOL"), &colorval) && colorval != wxT(""))
			if(CcolorS(colorval, colorstr))
				brkptcol = colorstr;
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/BRKPTCOL"), &colorval) && colorval != wxT(""))
				if(CcolorS(colorval, colorstr))
					brkptcol = colorstr;
				else
					brkptcol = wxT("#FF0000");
		...
		if(colorconf->Read(path + wxT("ACTBRKPTCOL"), &colorval) && colorval != wxT(""))
			if(CcolorS(colorval, colorstr))
				actbrkptcol = colorstr;
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/ACTBRKPTCOL"), &colorval) && colorval != wxT(""))
				if(CcolorS(colorval, colorstr))
					actbrkptcol = colorstr;
				else
					actbrkptcol = wxT("#0000FF");
		...
		if(colorconf->Read(path + wxT("ERRORCOL"), &colorval) && colorval != wxT(""))
			if(CcolorS(colorval, colorstr))
				errorcol = colorstr;
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/ERRORCOL"), &colorval) && colorval != wxT(""))
				if(CcolorS(colorval, colorstr))
					errorcol = colorstr;
				else
					errorcol = wxT("#800000");
		...
		colorstyle = style.MakeLower();




		if(colorconf->Read(path + wxT("EDGCOLR"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetEdgeColour(color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/EDGCOLR"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
					this->SetEdgeColour(color);
				else
					this->SetEdgeColour(black);




		if(colorconf->Read(path + wxT("EDGCOLM"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetEdgeColumn(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/EDGCOLM"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetEdgeColumn(r);
			...
			this->SetEdgeColumn(50);




		if(colorconf->Read(path + wxT("EDGMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetEdgeMode(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/EDGMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetEdgeMode(r);
			...
			this->SetEdgeMode(XSTC_DEF(EDGE_LINE));




		if(colorconf->Read(path + wxT("WRAPMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetWrapMode(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WRAPMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetWrapMode(r);
			...
			this->SetWrapMode(XSTC_DEF(WRAP_WORD));




		if(colorconf->Read(path + wxT("WRAPVF"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetWrapVisualFlags(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WRAPVF"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetWrapVisualFlags(r);
			...
			this->SetWrapVisualFlags(XSTC_DEF(WRAPVISUALFLAG_END));




		if(colorconf->Read(path + wxT("CACHEMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetLayoutCache(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CACHEMOD"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetLayoutCache(r);
			...
			this->SetLayoutCache(XSTC_DEF(CACHE_CARET));




		if(colorconf->Read(path + wxT("CARETLV"), &colorval) && colorval != wxT(""))
		{
			colorval.MakeLower();
			bool caret = (colorval == wxT("true") || colorval == wxT("1"));
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CARETLV"), &colorval) && colorval != wxT(""))
		{
			colorval.MakeLower();
			bool caret = (colorval == wxT("true") || colorval == wxT("1"));
...
#ifndef XSTC_NO_ALPHA
		if(colorconf->Read(path + wxT("ALPHALVL"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			alphalvl = r;
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/ALPHALVL"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			alphalvl = r;
		...
		if(colorconf->Read(path + wxT("MARKALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->MarkerSetAlpha(0, r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/MARKALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->MarkerSetAlpha(0, r);
		...
		if(colorconf->Read(path + wxT("SELALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetSelAlpha(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/SELALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetSelAlpha(r);
		...
		if(colorconf->Read(path + wxT("CARETALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetCaretALPHA(r);
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CARETALPHA"), &colorval) && colorval != wxT(""))
		{
			colorval.ToLong(&n,10);
			r=n;
			this->SetCaretALPHA(r);
		...
		if(colorconf->Read(path + wxT("WS_INVISIBLE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_INVISIBLE), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/WS/INVISIBLE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_INVISIBLE), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(WS_INVISIBLE), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("WS_VISIBLEALWAYS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEALWAYS), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/WS/VISIBLEALWAYS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEALWAYS), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEALWAYS), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("WS_VISIBLEAFTERINDENT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEAFTERINDENT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/WS/VISIBLEAFTERINDENT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEAFTERINDENT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(WS_VISIBLEAFTERINDENT), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("EOL_CRLF"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_CRLF), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/EOL/CRLF"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_CRLF), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(EOL_CRLF), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("EOL_CR"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_CR), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/EOL/CR"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_CR), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(EOL_CR), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("EOL_LF"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_LF), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/EOL/LF"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(EOL_LF), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(EOL_LF), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("CP_DBCS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(CP_DBCS), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/CP/DBCS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(CP_DBCS), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(CP_DBCS), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARKER_MAX"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARKER_MAX), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARKER/MAX"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARKER_MAX), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARKER_MAX), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_CIRCLE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_CIRCLE), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/CIRCLE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_CIRCLE), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_CIRCLE), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_ROUNDRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ROUNDRECT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/ROUNDRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ROUNDRECT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_ROUNDRECT), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_ARROW"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROW), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/ARROW"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROW), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_ARROW), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_SMALLRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_SMALLRECT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/SMALLRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_SMALLRECT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_SMALLRECT), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_SHORTARROW"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_SHORTARROW), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/SHORTARROW"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_SHORTARROW), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_SHORTARROW), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_EMPTY"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_EMPTY), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/EMPTY"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_EMPTY), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_EMPTY), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_ARROWDOWN"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWDOWN), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/ARROWDOWN"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWDOWN), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWDOWN), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_MINUS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_MINUS), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/MINUS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_MINUS), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_MINUS), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_PLUS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_PLUS), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/PLUS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_PLUS), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_PLUS), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_BACKGROUND"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_BACKGROUND), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/BACKGROUND"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_BACKGROUND), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_BACKGROUND), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_DOTDOTDOT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_DOTDOTDOT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/DOTDOTDOT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_DOTDOTDOT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_DOTDOTDOT), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_ARROWS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWS), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/ARROWS"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWS), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_ARROWS), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_PIXMAP"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_PIXMAP), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/PIXMAP"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_PIXMAP), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_PIXMAP), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("MARK_FULLRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_FULLRECT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/FULLRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_FULLRECT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_FULLRECT), wxT("fore:#000000,back:#FFFFFF"));
#endif //XSTC_NO_MARK_FULLRECT
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/LEFTRECT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT), wxT("fore:#000000,back:#FFFFFF"));
#endif //XSTC_NO_MARK_LEFTRECT
		...
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/AVAILABLE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE), wxT("fore:#000000,back:#FFFFFF"));
#endif //XSTC_NO_MARK_AVAILABLE
		...
		if(colorconf->Read(path + wxT("MARK_CHARACTER"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_CHARACTER), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARK/CHARACTER"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(MARK_CHARACTER), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(MARK_CHARACTER), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_DEFAULT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_DEFAULT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/DEFAULT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_DEFAULT), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_DEFAULT), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_LINENUMBER"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_LINENUMBER), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/LINENUMBER"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_LINENUMBER), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_LINENUMBER), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_BRACELIGHT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_BRACELIGHT), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/BRACELIGHT"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_BRACELIGHT), colorval);
		else
		...
		if(colorconf->Read(path + wxT("STYLE_BRACEBAD"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_BRACEBAD), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/BRACEBAD"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_BRACEBAD), colorval);
		else
		...
		if(colorconf->Read(path + wxT("STYLE_CONTROLCHAR"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_CONTROLCHAR), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/CONTROLCHAR"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_CONTROLCHAR), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_CONTROLCHAR), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_INDENTGUIDE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_INDENTGUIDE), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/INDENTGUIDE"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_INDENTGUIDE), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_INDENTGUIDE), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_LASTPREDEFINED"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_LASTPREDEFINED), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/LASTPREDEFINED"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_LASTPREDEFINED), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_LASTPREDEFINED), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("STYLE_MAX"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_MAX), colorval);
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/STYLE/MAX"), &colorval) && colorval != wxT(""))
			this->StyleSetSpec(XSTC_DEF(STYLE_MAX), colorval);
		else
			this->StyleSetSpec(XSTC_DEF(STYLE_MAX), wxT("fore:#000000,back:#FFFFFF"));
		...
		if(colorconf->Read(path + wxT("FOLDFG"), &colorval) && colorval != wxT(""))
			if(CcolorS(colorval, colorstr))
				foldfg = colorstr;
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARKNUM/FOLDFG"), &colorval) && colorval != wxT(""))
				if(CcolorS(colorval, colorstr))
					foldfg = colorstr;
				else
					foldfg = wxT("#FFFFFF");
		...
		if(colorconf->Read(path + wxT("FOLDBG"), &colorval) && colorval != wxT(""))
			if(CcolorS(colorval, colorstr))
				foldbg = colorstr;
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MARKNUM/FOLDBG"), &colorval) && colorval != wxT(""))
				if(CcolorS(colorval, colorstr))
					foldbg = colorstr;
				else
					foldbg = wxT("#000000");
		...
		if(colorconf->Read(path + wxT("MISIC_WSFG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetWhitespaceForeground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WSFG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
		...
		if(colorconf->Read(path + wxT("MISIC_WSBG"), &colorval) && colorval != wxT(""))
			if(Ccolor(colorval, color))
				this->SetWhitespaceForeground(true, color);
			else if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/WSBG"), &colorval) && colorval != wxT(""))
				if(Ccolor(colorval, color))
		...
		if(colorconf->Read(path + wxT("XS_COMMENT"), &colorval) && colorval != wxT(""))
			XS_comment = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_COMMENT"), &colorval) && colorval != wxT(""))
			XS_comment = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_COMMENT2"), &colorval) && colorval != wxT(""))
			XS_comment2 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_COMMENT2"), &colorval) && colorval != wxT(""))
			XS_comment2 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_COMMENT3"), &colorval) && colorval != wxT(""))
			XS_comment3 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_COMMENT3"), &colorval) && colorval != wxT(""))
			XS_comment3 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_S_STRING"), &colorval) && colorval != wxT(""))
			XS_s_string = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_S_STRING"), &colorval) && colorval != wxT(""))
			XS_s_string = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_D_STRING"), &colorval) && colorval != wxT(""))
			XS_d_string = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_D_STRING"), &colorval) && colorval != wxT(""))
			XS_d_string = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_NUMBER"), &colorval) && colorval != wxT(""))
			XS_number = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_NUMBER"), &colorval) && colorval != wxT(""))
			XS_number = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_CHAR"), &colorval) && colorval != wxT(""))
			XS_char = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_CHAR"), &colorval) && colorval != wxT(""))
			XS_char = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_DEFAULT"), &colorval) && colorval != wxT(""))
			XS_default = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_DEFAULT"), &colorval) && colorval != wxT(""))
			XS_default = colorval;
		else
			XS_default = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_KEY1"), &colorval) && colorval != wxT(""))
			XS_key1 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_KEY1"), &colorval) && colorval != wxT(""))
			XS_key1 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_KEY2"), &colorval) && colorval != wxT(""))
			XS_key2 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_KEY2"), &colorval) && colorval != wxT(""))
			XS_key2 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_KEY3"), &colorval) && colorval != wxT(""))
			XS_key3 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_KEY3"), &colorval) && colorval != wxT(""))
			XS_key3 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_KEY4"), &colorval) && colorval != wxT(""))
			XS_key4 = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_KEY4"), &colorval) && colorval != wxT(""))
			XS_key4 = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_PREPROC"), &colorval) && colorval != wxT(""))
			XS_preproc = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_PREPROC"), &colorval) && colorval != wxT(""))
			XS_preproc = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_SYMBOL"), &colorval) && colorval != wxT(""))
			XS_symbol = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_SYMBOL"), &colorval) && colorval != wxT(""))
			XS_symbol = colorval;
		else
			XS_symbol = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_TAG"), &colorval) && colorval != wxT(""))
			XS_tag = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_TAG"), &colorval) && colorval != wxT(""))
			XS_tag = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_USER"), &colorval) && colorval != wxT(""))
			XS_user = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_USER"), &colorval) && colorval != wxT(""))
			XS_user = colorval;
		else
			XS_user = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_MISIC"), &colorval) && colorval != wxT(""))
			XS_misic = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_MISIC"), &colorval) && colorval != wxT(""))
			XS_misic = colorval;
		else
			XS_misic = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_LANG"), &colorval) && colorval != wxT(""))
			XS_lang = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_LANG"), &colorval) && colorval != wxT(""))
			XS_lang = colorval;
		else
			XS_lang = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_GLOBAL"), &colorval) && colorval != wxT(""))
			XS_global = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_GLOBAL"), &colorval) && colorval != wxT(""))
			XS_global = colorval;
		else
			XS_global = wxT("fore:#000000,back:#FFFFFF");
		...
		if(colorconf->Read(path + wxT("XS_BAD"), &colorval) && colorval != wxT(""))
			XS_bad = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_BAD"), &colorval) && colorval != wxT(""))
			XS_bad = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_INSTRUCTION"), &colorval) && colorval != wxT(""))
			XS_instruction = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_INSTRUCTION"), &colorval) && colorval != wxT(""))
			XS_instruction = colorval;
		else
		...
		if(colorconf->Read(path + wxT("XS_DTYPE"), &colorval) && colorval != wxT(""))
			XS_dtype = colorval;
		else if(colorconf->Read(confroot + wxT("XSTC/COLOR/XS/XS_DTYPE"), &colorval) && colorval != wxT(""))
			XS_dtype = colorval;
		else
...
{//This returns the names of themes stored in the config database if Cstyles was defined.
	wxString colorval, temp;
	if(colorconf->Read(confroot + wxT("XSTC/COLOR/MISIC/CSTYLES"), &colorval) && colorval != wxT(""))
	{
		colorval.Trim(true);
		colorval.Trim(false);
		colorval = colorval.Upper();
		CStyles = wxT("");
		wxStringTokenizer tkz(colorval, wxT(","));
...
{//Reverts everything back to default black on white.
	this->StyleClearAll();
	brkptcol     = wxT("#FF0000");
	actbrkptcol  = wxT("#0000FF");
	errorcol     = wxT("#800000");
...
#endif //XSTC_NO_MARK_FULLRECT
#ifndef XSTC_NO_MARK_LEFTRECT
	this->StyleSetSpec(XSTC_DEF(MARK_LEFTRECT),   XS_default);
#endif //XSTC_NO_MARK_LEFTRECT
#ifndef XSTC_NO_MARK_AVAILABLE
	this->StyleSetSpec(XSTC_DEF(MARK_AVAILABLE),   XS_default);
#endif //XSTC_NO_MARK_AVAILABLE
...
{//Split a property string from the extension config file.
	PropSTR result;
	result.L = wxT("");
	result.R = wxT("");
	if(!(property.IsEmpty()))
...
{//Get a lexer based on a file extension
	extparam.Trim(true);
	extparam.Trim(false);
	...
	{
		//If a number is passed, that is returned verbatim
		long res;
...
//js uses the cpp lexer, if it is turned off, js will not work either. same with java.
#ifndef XSTC_NO_CPP
	if(extparam==wxT("js"))
		return XSTC_DEF(LEX_CPP);
...
{//Validate a filename, make sure no invalid characters are present.
...
#ifndef XSTC_NO_CONFIG
bool XSTC::IsConfEXT(wxString extension, int &lexer)
{//Check if an extension was set in the config database, if so set lexer appropriatly.
	extension = extension.Trim(false);
	extension = extension.Trim(true);
	...
	if(colorconf)
	{
		if(colorconf->Read(confroot + wxT("XSTC/EXT/") + extension + wxT("/LEXER"), &val) && val != wxT(""))
		{
			if(val.IsNumber())
			{
				val.ToLong(&lexnum, 10);
				lexer = lexnum;
				if(lexer != XSTC_DEF(LEX_NULL))
					return true;
			}
			else
			{
				lexer = GetEXTLexer(val);
				if(lexer != XSTC_DEF(LEX_NULL))
					return true;
			}
		}
...
bool XSTC::SetProperties(wxString extension)
{//Check the config database for properties for this extention and set them.
	extension = extension.Trim(false);
	extension = extension.Trim(true);
	extension = extension.Upper();
	wxString val, place, tmp;
	PropSTR property;
	long num;
	if(colorconf)
	{
		...
		else if(colorconf->Read(confroot + wxT("XSTC/EXT/") + extension + wxT("/PROPCOUNT"), &val) && val != wxT(""))
		{
			if(val.IsNumber())
			{
				val.ToLong(&num, 10);
...
{//May not be too useful due to scintilla's internals, but you can change what margin does what.
	linemgn = line;
	symmgn = symbol;
	fldmgn = fold;
...
{//Set the width of all three margins at once.
...
{//Trims trailing spaces in the document.
...
{//Converts between spaces and tabs with spaces.
...
{//Set a config database for configuration based coloring.
	if(clrconf)
	{
		if(colorconf)
			delete colorconf;
...
wxConfigBase* XSTC::SetColorConf(wxConfigBase* clrconf, bool pass, wxString root)
{//An overloaded version that will return the current database.
	if(clrconf)
	{
		wxConfigBase* base = NULL;
		if(colorconf)
			base = colorconf;
		confroot = root;
		colorconf = clrconf;
...
{//Resets all XSTC managed (0-10) markers back to defaults.
	this->MarkerDefine(0, markshape);
...
{//Convert a string color entry to wxColor.
	bool result = false;
	...
	else if(configvalue.Contains(wxT("#")))
	{
...
{//Convert a wxColour entry to a string representation
	bool result = false;
...
{//Set the color database so color names can be used in the config database.
	if(XSTCcolorDbase)
	{
...
wxColourDatabase* XSTC::SetColorDbase(wxColourDatabase& dbase, bool pass)
{//Overloaded version that will return the old database.
	if(pass)
	{
		wxColourDatabase* db = NULL;
		if(XSTCcolorDbase)
		{
			db = XSTCcolorDbase;
		}
