...
wxJSONReader::DoRead( wxJSONValue& parent )
{
  ...
  do {                   // we read until ch < 0
    switch ( ch )  {
      ...
      case '}' :
        if ( !parent.IsObject() )  {
          AddWarning( _T("Trying to close an array using the \'}\' (close-object) char" ));
        }
      ...
      case ']' :
        if ( !parent.IsArray() )  {
          AddWarning( _T("Trying to close an object using the \']\' (close-array) char" ));
        }
        StoreValue( ch, key, value, parent );
        m_current = &parent;
        m_next    = 0;
  ...
  if ( parent.IsArray() )  {
    AddWarning( _T("\']\' missing at end of file"));
  }
...
/*!
 ...
 \code
   Warning: line xxx, col xxx - <error_description>
 \endcode

 Note that if the \c m_strict flag is set to TRUE, than the
 function calls AddError().
*/
void
wxJSONReader::AddWarning( const wxString& msg )
{
  if ( m_flags == wxJSONREADER_STRICT )  {
    AddError( msg );
    return;
  }
...
wxJSONReader::ReadString( wxJSONValue& val )
{
  ...
  if ( val.IsEmpty() )   {
    ...
    AddWarning( _T("Multiline strings are not allowed by JSON syntax") );
  ...
  switch ( ch )  {
    ...
    default: 
      if ( s == _T("null") ) {
        ...
        ::wxLogTrace( traceMask, _T("(%s) value = NULL"),  __PRETTY_FUNCTION__ );
        AddWarning( _T( "the \'null\' literal must be lowercase" ));
        val.SetType( wxJSONTYPE_NULL );
        return nextCh;
      }
      else if ( s == _T("true") ) {
        ...
        ::wxLogTrace( traceMask, _T("(%s) value = TRUE"),  __PRETTY_FUNCTION__ );
        AddWarning( _T( "the \'true\' literal must be lowercase" ));
        val = true;
        return nextCh;
      }
      else if ( s == _T("false") ) {
        ...
        ::wxLogTrace( traceMask, _T("(%s) value = FALSE"),  __PRETTY_FUNCTION__ );
        AddWarning( _T( "the \'false\' literal must be lowercase" ));
        val = false;
        return nextCh;
      }
      else {
  ...
  if ( m_flags & wxJSONREADER_COMMENTS_AFTER )  {  // comment AFTER
    if ( m_current )  {
      if ( m_current == parent || m_current->IsEmpty()) {
        AddWarning(_T("Cannot find a value for storing the comment (flag AFTER)"));
      }
      else  {
    ...
    if ( m_next )  {
     ::wxLogTrace( storeTraceMask, _T("(%s) comment added to m_next (BEFORE)"),
      ...
      AddWarning(_T("Cannot find a value for storing the comment (flag BEFORE)"));
    }
  }
  m_comment.clear();
}
