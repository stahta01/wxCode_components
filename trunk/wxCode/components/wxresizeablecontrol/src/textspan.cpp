...
// Created:     2005/8/19
// RCS-ID:      $Id: textspan.cpp,v 1.1 2005-09-11 12:54:06 frm Exp $
...
{	
	/*wxTextSpanTruncFlag f = CanBeSplit();
	...
	// set the text of the new span to the rightmost piece of this span
#if 0
	int tokeep = (m_size.x-wxTS_LEFTRIGHT_BORDER*2)/GetAveCharWidth() - 1;
#else
	//int tokeep = GetTruncateSize();
#endif
	...
	if (spanidx == m_input->m_nSpanWithCaret) {
		if (m_input->m_nCaretPos <= tokeep) {
			...
			// this function...
			m_input->m_nCaretPos -= tokeep;
			m_input->m_nSpanWithCaret++;
		}
	} else if (spanidx < m_input->m_nSpanWithCaret) {
