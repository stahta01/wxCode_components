...
void mmMultiButton::FindAndSetSize()
{
  ...
  { // Refresh and set size of button.
    int total_width =0,
        total_height=0;
    if (mDefaultBitmap != wxNullBitmap)
    {
    ...
    if (mLabelStr != "" && mDefaultBitmap != wxNullBitmap)
      h_extra += mMarginSize;
    total_width  += w_extra;
...
// Redraw label
{
  ...
  else
  {
    ...
    dc.SetTextForeground(*wxTheColourDatabase->FindColour("GREY"));
    dc.DrawText(mLabelStr, x,y);
...
// Draw all borders of the button.
{
  ...
  if (mStyle & wxSIMPLE_BORDER)
  {
    dc.SetPen(*wxThePenList->FindOrCreatePen(*wxTheColourDatabase->FindColour("GREY"),
	                                     1,wxSOLID));
