            ...
            m_subType(subType)
{
   if(m_mainType.IsNull() || m_subType.IsNull())
   {
      ...
      for (unsigned int i = 0; i < sizeof(g_mimeTypeMappings) / sizeof(wxMimeTypeMapping); i++)
      {
...
void wxMimePart::Encode(wxOutputStream& out)
{
   if(m_data.IsNull())
   {
      ...
      wxString result;
      result << "Content-Type: " << m_mainType << "/" << m_subType;

      if(! m_inline)
      {
         result << "; name=\"" << m_fileName.GetFullName() + "\"" << cr
                << "Content-Disposition: attachment; filename=\""
                << m_fileName.GetFullName() << "\"";
      }
      result << cr << "Content-Transfer-Encoding: base64" << cr << cr
