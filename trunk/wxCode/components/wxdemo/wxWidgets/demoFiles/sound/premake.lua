...
if ( windows ) then
	table.insert(package.files, "sound.rc" )
end
...
--******* WINDOWS SETUP ***********
	...
	if ( options["use-wx-static"] ) then
		...
		if ( target == "cb-gcc" or target == "gnu" ) then
			...
			table.insert( package.config["Debug"].links, { "rpcrt4", "comctl32", "winmm" } )
			table.insert( package.config["Release"].links, { "rpcrt4", "comctl32", "winmm" } )
		end
	end
