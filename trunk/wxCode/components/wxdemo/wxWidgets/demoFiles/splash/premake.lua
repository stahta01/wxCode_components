...
--******* LINUX/MAC SETUP *************
  ...
  if ( options["unicode"] ) then wx_config_params = wx_config_params.."yes"
	...
	table.insert( package.config["Debug"].linkoptions, wx_config_params.." --debug=yes --libs std,media`" )
	table.insert( package.config["Release"].linkoptions, wx_config_params.." --debug=no --libs std,media`" )
end
