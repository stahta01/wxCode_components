...
// Modified by:
// Created:     1995
// RCS-ID:      $Id: printing.cpp 48649 2007-09-13 12:23:39Z RR $
...
//    g_printData->SetPaperId(wxPAPER_A4);    // for everyone else    
    ...
    file_menu->Append(WX_LATEX, _T("&Latex"),                   _T("Export Latex/PsTricks"));
...
EVT_MENU(WX_LATEX, MyFrame::OnExportlatex)
...
void MyFrame::OnExportlatex(wxCommandEvent& WXUNUSED(event)) {
    wxFileDialog latexFileDialog( this, wxT("Save LaTeX file"), wxT(""), wxT(""),
        wxT("LaTeX file (*.tex)|*.tex"), wxFD_SAVE );
    if (latexFileDialog.ShowModal()!=wxID_OK) return;

    wxLatexDC latexDC(latexFileDialog.GetPath(),GetRect().width,GetRect().height,72.0);
    Draw(latexDC);
}
