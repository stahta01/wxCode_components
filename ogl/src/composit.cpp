...
wxCompositeShape::~wxCompositeShape()
{
  ...
  for (it = m_constraints.begin();
       ...
       it++)
  {
    wxOGLConstraint* constraint = wxStaticCast(*it, wxOGLConstraint);
    delete constraint;
  }
...
void wxCompositeShape::OnRightClick(double x, double y, int keys, int WXUNUSED(attachment))
{
  ...
  if (keys & KEY_CTRL)
  {
    for (wxObjectList::const_iterator it = m_divisions.begin();
         it != m_divisions.end();
         it++)
    {
      wxDivisionShape* division = wxStaticCast(*it, wxDivisionShape);
      int attach = 0;
      double dist = 0.0;
...
{
  for (wxObjectList::compatibility_iterator node = m_constraints.GetFirst();
       node;
       )
  {
    wxOGLConstraint* constraint = wxStaticCast(node->GetData(), wxOGLConstraint);
    wxObjectList::compatibility_iterator nextNode = node->GetNext();
...
void wxCompositeShape::Copy(wxShape& copy)
{
       ...
       it++)
  {
    ...
    wxShape* newConstraining = wxStaticCast(oglObjectCopyMapping.Find((long)constraint->m_constrainingObject)->GetData(),wxShape);

    wxList newConstrainedList;
    for (wxObjectList::const_iterator it2 = constraint->m_constrainedObjects.begin();
         it2 != constraint->m_constrainedObjects.end();
         it2++)
    {
      wxShape* constrainedObject = wxStaticCast(*it2, wxShape);
      wxShape* newConstrained = wxStaticCast(oglObjectCopyMapping.Find((long)constrainedObject)->GetData(), wxShape);
    ...
    if (constraint->m_constraintName.Length())
    {
  ...
  for (it = m_divisions.begin();
       it != m_divisions.end();
       it++)
  {
    wxDivisionShape *division = wxStaticCast(*it, wxDivisionShape);
    wxObjectList::compatibility_iterator node1 = oglObjectCopyMapping.Find((long)division);
    wxObjectList::compatibility_iterator leftNode = NULL;
    wxObjectList::compatibility_iterator topNode = NULL;
    wxObjectList::compatibility_iterator rightNode = NULL;
    wxObjectList::compatibility_iterator bottomNode = NULL;
    ...
    if (node1)
    {
      wxDivisionShape* newDivision = wxStaticCast(node1->GetData(), wxDivisionShape);
      if (leftNode)
        newDivision->SetLeftSide(wxStaticCast(leftNode->GetData(), wxDivisionShape));
      if (topNode)
        newDivision->SetTopSide(wxStaticCast(topNode->GetData(), wxDivisionShape));
      if (rightNode)
        newDivision->SetRightSide(wxStaticCast(rightNode->GetData(), wxDivisionShape));
      if (bottomNode)
        newDivision->SetBottomSide(wxStaticCast(bottomNode->GetData(), wxDivisionShape));
    }
  }
}
...
wxOGLConstraint *wxCompositeShape::FindConstraint(long cId, wxCompositeShape **actualComposite)
{
       ...
       it++)
  {
    ...
    if (wxDynamicCast(child, wxCompositeShape))
    {
      ...
      {
        if (actualComposite)
          *actualComposite = wxStaticCast(child, wxCompositeShape);
        return constraint;
      }
    }
  }
...
//  clause->AddAttributeValue("selectable", (long)selectable);
  ...
  for (wxObjectList::compatibility_iterator node = m_constraints.GetFirst();
       node;
       node = node->GetNext(), constraintNo++)
  {
    wxOGLConstraint *constraint = wxStaticCast(node->GetData(), wxOGLConstraint);
    ...
    for (wxObjectList::compatibility_iterator node1 = constraint->m_constrainedObjects.GetFirst();
         node1;
         node1 = node1->GetNext())
    {
      wxShape* obj = wxStaticCast(node1->GetData(), wxShape);
  ...
  for (node = m_children.GetFirst();
       node;
       node = node->GetNext())
  {
    wxShape* child = wxStaticCast(node->GetData(), wxShape);
...
bool wxDivisionShape::Divide(int direction)
{
  ...
  double x1 = (double)(GetX() - (GetWidth()/2.0));
  double y1 = (double)(GetY() - (GetHeight()/2.0));
  wxCompositeShape* compositeParent = wxStaticCast(GetParent(), wxCompositeShape);
  ...
  if (direction == wxVERTICAL)
  {
    ...
    for (wxObjectList::const_iterator it = compositeParent->GetDivisions().begin();
         it != compositeParent->GetDivisions().end();
         it++)
    {
      wxDivisionShape* obj = wxStaticCast(*it, wxDivisionShape);
...
*/
  ...
  {
    wxDivisionControlPoint* control = wxStaticCast(node->GetData(), wxDivisionControlPoint);
...
{
  wxDivisionShape* division = wxStaticCast(m_shape, wxDivisionShape);
...
void wxDivisionControlPoint::OnEndDragLeft(double x, double y, int keys, int attachment)
{
  ...
  wxDivisionShape* division = wxStaticCast(m_shape, wxDivisionShape);
  wxCompositeShape* divisionParent = wxStaticCast(division->GetParent(), wxCompositeShape);
...
{
  wxCompositeShape* divisionParent = wxStaticCast(GetParent(),wxCompositeShape);
  for (wxObjectList::const_iterator it = divisionParent->GetDivisions().begin();
       it != divisionParent->GetDivisions().end();
       it++)
  {
    wxDivisionShape* division = wxStaticCast(*it, wxDivisionShape);
    switch (side)
...
{
  wxDivisionShape* division = wxStaticCast(GetClientData(), wxDivisionShape);
  switch (event.GetInt())
  {
