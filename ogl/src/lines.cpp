...
void wxLineShape::FormatText(wxDC& dc, const wxString& s, int i)
{
  ...
  for (node = (wxNode*)string_list->GetFirst();
       node;
       node = node->GetNext())
  {
...
void wxLineShape::Straighten(wxDC *dc)
{
  ...
  for (wxNode* node = first_point_node;
...
bool wxLineShape::HitTest(double x, double y, int *attachment, double *distance)
{
  ...
  for (wxNode* node = m_lineControlPoints->GetFirst();
       node && node->GetNext();
       node = node->GetNext())
  {
...
void wxLineShape::DrawArrows(wxDC& dc)
{
  ...
  for (wxNode* node = m_arcArrows.GetFirst();
       node;
       node = node->GetNext())
  {
...
void wxLineShape::FindNth(wxShape *image, int *nth, int *no_arcs, bool incoming)
{
  ...
  for (wxNode* node = image->GetLines().GetFirst();
       node;
       node = node->GetNext())
  {
...
void wxLineShape::Copy(wxShape& copy)
{
  ...
  wxNode* node;
  for (node = m_lineOrientations.GetFirst();
       node;
       node = node->GetNext())
  {
  ...
  for (node = m_lineControlPoints->GetFirst();
       node;
       node = node->GetNext())
  {
    wxRealPoint *point = (wxRealPoint *)node->GetData();
...
double wxLineShape::FindMinimumWidth()
{
  ...
  for (wxNode* node = m_arcArrows.GetFirst();
       ...
       node = node->GetNext())
  {
    wxArrowHead* arrowHead = wxStaticCast(node->GetData(), wxArrowHead);
...
bool wxLineShape::OnLabelMovePre(wxDC& dc, wxLabelShape* labelShape, double x, double y, double WXUNUSED(old_x), double WXUNUSED(old_y), bool WXUNUSED(display))
{
  ...
  int i = 0;
  for (wxNode* node = GetRegions().GetFirst();
       node;
       )
  {
