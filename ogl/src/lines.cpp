...
wxObjectList::compatibility_iterator wxLineShape::InsertLineControlPoint(wxDC* dc)
{
    if (dc)
        Erase(*dc);

  wxObjectList::compatibility_iterator last = m_lineControlPoints->GetLast();
  wxObjectList::compatibility_iterator second_last = last->GetPrevious();
  ...
  wxObjectList::compatibility_iterator node = m_lineControlPoints->Insert(last, (wxObject*) point);
  return node;
}
...
void wxLineShape::Initialise()
{
  if (m_lineControlPoints)
  {
    ...
    wxObjectList::compatibility_iterator first = m_lineControlPoints->GetFirst();
    ...
    // and the last.
    for (wxObjectList::compatibility_iterator node = first->GetNext();
         node;
         node = node->GetNext())
    {
...
void wxLineShape::GetLabelPosition(int position, double *x, double *y)
{
  switch (position)
  {
    case 0:
    {
      ...
      wxObjectList::compatibility_iterator node = m_lineControlPoints->Item(half_way - 1);
      wxRealPoint *point = (wxRealPoint *)node->GetData();
      wxObjectList::compatibility_iterator next_node = node->GetNext();
    ...
    case 1:
    {
      wxObjectList::compatibility_iterator node = m_lineControlPoints->GetFirst();
    ...
    case 2:
    {
      wxObjectList::compatibility_iterator node = m_lineControlPoints->GetLast();
      *x = ((wxRealPoint *)node->GetData())->x;
      *y = ((wxRealPoint *)node->GetData())->y;
      break;
    }
...
void wxLineShape::Straighten(wxDC *dc)
{
  ...
  wxObjectList::compatibility_iterator first_point_node = m_lineControlPoints->GetFirst();
  wxObjectList::compatibility_iterator last_point_node = m_lineControlPoints->GetLast();
  wxObjectList::compatibility_iterator second_last_point_node = last_point_node->GetPrevious();
  ...
  for (wxObjectList::compatibility_iterator  node = first_point_node;
       node && (node != second_last_point_node);
       node = node->GetNext())
  {
    wxRealPoint *point = (wxRealPoint *)node->GetData();
...
bool wxLineShape::HitTest(double x, double y, int *attachment, double *distance)
{
  ...
  for (int i = 0; i < 3; i ++)
  {
    wxObjectList::compatibility_iterator regionNode = m_regions.Item(i);
    if (regionNode)
    {
      wxShapeRegion* region = wxStaticCast(regionNode->GetData(), wxShapeRegion);
  ...
  for (wxObjectList::compatibility_iterator node = m_lineControlPoints->GetFirst();
       node && node->GetNext();
       node = node->GetNext())
  {
...
void wxLineShape::DrawArrows(wxDC& dc)
{
  ...
  for (wxObjectList::const_iterator it = m_arcArrows.begin();
       ...
       it++)
  {
    wxArrowHead* arrow = wxStaticCast(*it, wxArrowHead);
...
{
  wxObjectList::compatibility_iterator first_line_node = m_lineControlPoints->GetFirst();
  wxRealPoint *first_line_point = (wxRealPoint *)first_line_node->GetData();
  wxObjectList::compatibility_iterator second_line_node = first_line_node->GetNext();
  wxRealPoint *second_line_point = (wxRealPoint *)second_line_node->GetData();

  wxObjectList::compatibility_iterator last_line_node = m_lineControlPoints->GetLast();
  wxRealPoint *last_line_point = (wxRealPoint *)last_line_node->GetData();
  wxObjectList::compatibility_iterator second_last_line_node = last_line_node->GetPrevious();
...
void wxLineShape::FindLineEndPoints(double *fromX, double *fromY, double *toX, double *toY)
{
  ...
  wxObjectList::compatibility_iterator second = first->GetNext();
...
void wxLineShape::ResetControlPoints()
{
  if (m_canvas && m_lineControlPoints && m_controlPoints.GetCount() > 0)
  {
    ...
    wxObjectList::compatibility_iterator control_node = m_lineControlPoints->GetFirst();
    for (;
         node && control_node;
         node = node->GetNext(), control_node = control_node->GetNext())
    {
      wxRealPoint *point = (wxRealPoint *)control_node->GetData();
      wxLineControlPoint* control = wxStaticCast(node->GetData(), wxLineControlPoint);
...
void wxLineShape::Copy(wxShape& copy)
{
  ...
  for (it = m_lineControlPoints->begin();
       it != m_lineControlPoints->end();
       it++)
  {
    wxRealPoint *point = (wxRealPoint*)*it;
...
{
  wxLineControlPoint* lpt = wxStaticCast(pt, wxLineControlPoint);
  ...
  if (lpt->m_type == CONTROL_POINT_LINE)
  {
    ...
    wxLineShape* lineShape = wxStaticCast(this, wxLineShape);
...
{
  wxObjectList::compatibility_iterator refNode = referenceList.GetFirst();
  wxObjectList::compatibility_iterator currNode = m_arcArrows.GetFirst();
  ...
  wxArrowHead *refArrow = wxStaticCast(refNode->GetData(), wxArrowHead);
  ...
  for (wxArrowHead *currArrow = (wxArrowHead *)currNode->GetData();
       refNode && currNode;
       refNode = refNode->GetNext())
  {
    refArrow = wxStaticCast(refNode->GetData(), wxArrowHead);
        ...
        (currArrow->GetName() == refArrow->GetName()))
    {
      ...
      if (currNode)
        currArrow = wxStaticCast(currNode->GetData(), wxArrowHead);
    }
...
{
  for (wxObjectList::compatibility_iterator node = m_arcArrows.GetFirst();
       node;
       )
  {
...
double wxLineShape::FindMinimumWidth()
{
  ...
  for (wxObjectList::const_iterator it = m_arcArrows.begin();
       ...
       it++)
  {
    wxArrowHead* arrowHead = wxStaticCast(*it, wxArrowHead);
    minWidth += arrowHead->GetSize();
    if ((it++) != m_arcArrows.end())
    {
...
wxRealPoint *wxLineShape::GetNextControlPoint(wxShape *nodeObject)
{
  ...
  wxObjectList::compatibility_iterator node = m_lineControlPoints->Item(nn);
  if (node)
  {
...
bool wxLineShape::OnLabelMovePre(wxDC& dc, wxLabelShape* labelShape, double x, double y, double WXUNUSED(old_x), double WXUNUSED(old_y), bool WXUNUSED(display))
{
  ...
  {
    if (labelShape->m_shapeRegion == wxStaticCast(node->GetData(), wxShapeRegion))
    {
      node = NULL;
    }
    else
